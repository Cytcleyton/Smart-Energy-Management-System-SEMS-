...
public class SolarPanelServiceImpl extends SolarPanelServiceGrpc.SolarPanelServiceImplBase {
        ...
        Thread streamer = new Thread(() -> {
            ...
            try {
                while (!cancelled.get() && !serverObserver.isCancelled()) {
                    handleNextReading(start, serverObserver);
                    Thread.sleep(1000);
                }
    ...
    private static void handleNextReading(long start, ServerCallStreamObserver<EnergyReading> serverObserver) {
        double elapsedSeconds = (System.currentTimeMillis() - start) / 1000.0;
        // Simulated weather/time-of-day factor: a smooth curve plus random noise.
